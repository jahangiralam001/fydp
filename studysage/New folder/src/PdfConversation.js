...
function PdfConversation() {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8000/internal_chats/${encodeURIComponent(user_id)}/${encodeURIComponent(chatName)}`);
    ...
    >
      ...
      <div className="chat-container">
        <div className="side-menu">
          ...
          {newChatName && (
            <button onClick={() => setchatName(newChatName)}>
              {newChatName}
            </button>
          )}
